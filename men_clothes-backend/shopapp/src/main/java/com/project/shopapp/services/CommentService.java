...
public class CommentService implements ICommentService {
    ...
    public List<CommentResponse> getCommentsByProduct(Long productId) {
        List<Comment> comments = commentRepository.findTop5ByProductIdOrderByCreatedAtDesc(productId);
        return comments.stream()
                .map(comment -> CommentResponse.fromComment(comment))
                .collect(Collectors.toList());
    }
    ...
    public List<CommentResponse> getCommentsByUser(Long userId) {
        List<Comment> comments = commentRepository.findTop5ByUserIdOrderByCreatedAtDesc(userId);
        return comments.stream()
                .map(comment -> CommentResponse.fromComment(comment))
                .collect(Collectors.toList());
    }
